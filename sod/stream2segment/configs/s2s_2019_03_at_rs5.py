...
_path = os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(__file__))),
                     'inventories', 's2s_2017_10')
...
WRONG_OLD_INVENTORIES = {
    ...
    890: (
        read_inventory(os.path.join(_path, "FR.PYLO.2010-01-17T10:00:00.xml")),
...
def get_traces_from_segment(segment, config):
    ...
    # overwritten stuff:
    if segment.data_seed_id in config['good']:
        params[HANDLABELLED_COL] = True
    ret = [(tra, inv, params)]
    wrong_old_inv = WRONG_OLD_INVENTORIES.get(segment.station.id, None)
    if wrong_old_inv is not None:
        oldinv, seed_ids = wrong_old_inv
        if segment.data_seed_id in seed_ids:
            ret.append(
                (tra, oldinv, {
                    **params, OUTLIER_COL: True, HANDLABELLED_COL: True
                })
            )
    return ret
